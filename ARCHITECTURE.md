...
这个入口不接受候选池参数。切换雷达池只影响随后运行哪一种分析，不缩小全量底层刷新范围。Flask 使用 `sys.executable` 直接启动 Python 编排器，不经过 Shell。直连模式通过同一公共函数按大小写不敏感方式清理当前进程和子进程的 HTTP、HTTPS、ALL_PROXY，并设置 `NO_PROXY=*`；`STOCK_CRAWL_NO_PROXY=1` 与 CLI `--no-proxy` 语义一致。

细分龙头行情与游资小盘建池子进程都通过临时 JSON sidecar 把逐股失败交回编排器，父进程再写入 `data/stock_data_refresh_report.json`。每只股票在单轮并发抓取中只处理一次；抓取异常、空行情、基本面不完整和单线程写库异常会在写线程结束后按原池顺序汇总。报告顶层 `failures[]` 与对应步骤的 `meta.failure_details[]` 保留本轮失败的 `code`、`name`、`stage`、`error`，CLI 在完整 JSON 后逐条输出全部失败明细。子进程异常和空股票池仍作为无法归属到单股的全局失败。sidecar 使用独立临时目录和参数列表传递，兼容 Windows 路径与文件锁规则。
...
fund/funds.py
  ...
  -> fund/ttjj_spider 基金概况 + fund/fund_fetch_data.py
  -> data/fund_data.sqlite3
  -> fund/fund_technical_analysis.py
  -> data/signals.json
  -> fund/fund_generate_output.py
  -> data/fund_report_data.json
  -> Flask /fund
```

`fund/funds.py` 是用户配置源。页面编辑器不会执行任意 Python，而是用 AST 只接受固定名称的六位基金代码列表和兼容样板，再原子写回 `fund/funds.py` 与 `data/fund_codes.json`。基金刷新和配置保存共享资源锁，防止运行中改动抓取集合。根级 `fund_data_refresh.py` 固定以仓库根为工作目录，先重复执行同一套安全校验，再用当前 Python 的 `-m scrapy` 和 `python -m fund...` 参数列表依次启动各步骤。缓存清理使用 Python 标准库，因此不依赖 `stat`、`date`、`rm` 或 Bash。网页基金刷新固定传入 `--no-proxy`；该模式除清理进程环境外，还让 Requests 使用 `trust_env=False` 并关闭 Scrapy 的 `HTTPPROXY_ENABLED`，保证两条抓取链都不回读系统代理。
...
长线因子的数据刷新集合等于 `leader ∪ hotmoney`：龙头数据在前段刷新，游资小盘数据在本轮成员筛选完成后补齐。基本面刷新计划以时间戳和三类必需数据完整性共同决定，财务指标主源为空或字段不兼容时使用东财兼容源。小盘补齐完成后还会重新校验全部最终成员；任一成员失败或仍不完整时不切换 `is_hot_money` 标记，也不生成新的策略快照。策略诊断同时返回当前正权重口径的加权覆盖，以及财务/估值、价量、资金面等分项覆盖；缺失因子的默认分仍用于降级评分，但页面会明确暴露其覆盖缺口。
...
A 股日线统一经过 `stock_crawl_common.fetch_qfq_daily_records()`，按新浪、腾讯、东财的顺序故障转移；各适配器都输出前复权 OHLCV，成交量统一为“手”、成交额统一为“元”，涨跌幅按相邻前复权收盘校正，并把实际尝试来源交给刷新审计。刷新器先批量读取 `stock_meta.listing_date`，仅对缺失股票调用交易所批量清单并持久化结果，随后把所有历史请求起点收紧到不早于上市日。向左补历史时包含本地最早有效交易日作为锚点；锚点存在时可合并已返回记录，但只有响应同时包含请求起点的完整 OHLCV，才把该起点写入 `stock_meta.history_coverage_start_date`。普通增量更新只允许边界向更早日期推进；全量替换会精确重置该边界，未在本轮重新证明则清空，避免静默截断被永久当成完整历史。锚点缺失、初次抓取和最新区间空响应仍是严格失败。连续失败的数据源只会在本轮暂时熔断，批次结束后不会再对失败股票发起第二轮请求。ETF 先走基金专用东财接口，再回退通用源，且不会进入股票上市日期补齐链路。
...
三级行业热度是独立离线报告：`sw3_industry_heat.py` 将所有有效 SW3 指数对齐到同一截止日和最近 20 个共同交易日，按成交额生成全部有效行业的热门排名；同时保留“末 5 日成交份额增长且 20 日趋势相关性为正”的升温集合，并用完全对称的口径生成“份额下降且趋势相关性为负”的降温集合。升温分为正、降温分为负，两者合并为趋势排名；方向冲突或缺少可比值的行业不进入趋势榜。历史以申万指数日频数据为主；趋势接口停更的行业可由官方指数分析日表的成交额份额透明估算，行业与日度点都带 `amount_is_estimate`。份额接口暂时为空时只续用上一份已通过完整性校验的估算点，不续用旧 AkShare 派生点。
...
| 表 | 作用 |
| --- | --- |
| `stock_meta` | 以六位代码为稳定主键；保存名称、`instrument_type`、`listing_date`、普通增量只向更早推进且全量替换时重新验证的 `history_coverage_start_date`、抓取元数据、基本面/分红等 JSON blob |
...
`stock` 与 `etf` 可以共用 `stock_history`，但必须由 `stock_meta.instrument_type` 隔离。`list_codes()`、`codes_with_history()` 和 `iter_history()` 默认只返回股票；需要全部证券时必须显式传 `instrument_type=None`。股票上市日期补齐只处理 `instrument_type=stock`，不得把 ETF 代码发送给股票交易所清单接口。
...
`data/stock_data_refresh_report.json` 是跨步骤刷新审计记录。除命令、退出码、耗时和健康检查外，它会汇总本轮失败股票的证券代码、名称、失败阶段与异常内容，并由 CLI 在任务末尾输出全部明细；日线错误还包含请求用途、区间和实际尝试来源，龙头及游资小盘子进程的逐股错误通过临时 sidecar 合并。上市日期获取或匹配失败使用阶段名 `listing_date`，无法归属到单股的股票池构建或编排异常使用 `code: null`。策略产物只有在所有依赖步骤成功后才允许重建。

`data/capital/sw3_industry_heat.json` 使用 `sw3_industry_heat.v3`：包含统一交易日窗口、单位/方法、逐日来源与数据质量、全行业明细，以及不复制 20 日序列的热门、升温、降温和合并趋势紧凑排名。热门排名必须覆盖全部有效行业；升温/降温排名必须分别与正向/负向候选集合完全一致，合并趋势排名必须等于两者并集且按有符号趋势分连续排序，三个趋势榜都允许候选数为零。只有截止日等于已校验的预期交易日、有效行业覆盖至少 80%、全量明细和连续排名都通过校验时，才通过同目录临时文件原子替换；否则保留上一份完整报告。页面读取兼容 v1/v2 快照，并从已有行业指标补充下降展示。
...
9. `fund/funds.py` 页面编辑只允许静态、经过校验的代码列表，不得恢复为执行任意用户 Python。
10. 研究有效性标签只适用于实际验证过的池和时间范围；股票结论不能自动标成 ETF 已验证。
11. SW3 行业热度必须使用同一截止日和同一组 20 个交易日；缺日、滞后和非法成交额不得补零。AkShare/Sina 成分汇总只允许补紧邻的一个已收盘交易日，且必须显式保留来源与推导标记。热门排名必须覆盖全部有效行业；升温、降温排名必须分别与正向、负向候选集合完全一致，合并趋势排名必须覆盖两者并集并保持有符号分数降序。市值缺失要保留 `None` 与覆盖率，不能伪装成完整总市值。
