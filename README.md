...
游资雷达上线可用的 `/radar` 页面，补齐吸筹/出货评分、题材热度、K 线详情、预警隐藏、大盘切换和 TopN 导出。A 股长线候选与优化器统一使用 SW3 细分行业龙头池，市值/指数相关因子默认不参与搜索，并新增策略走势、导出、板块 SQLite、题材候选、行业周期引擎和成交量单位修复工具。

#### Update v3.3.1  2026.6.22
修正长线策略与 optimizer 的实盘一致性：默认 TopN、PIT 可交易性、回测收益路径、策略走势、换手统计和图内持仓均跟随当前参数。游资雷达收敛机会分口径，补充二级行业热度、市值、相似行业、出货预警和相关爬虫/测试。

#### Update v3.3.2  2026.6.25
优化策略与 optimizer 性能：长线因子计算改用预转换、numpy 缓存和向量化，PIT 因子矩阵预构并支持 `OPTUNA_TRIAL_JOBS` 实验并行。刷新链路减少整表重写和 JSON 往返，日线 append 改增量 upsert，并补充 SQLite 与批量市值同步优化。
