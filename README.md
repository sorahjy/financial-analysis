...
> 注意：受本地约 10 年日线与基准 ETF 覆盖区间所限，PIT 有效折数有限，超额数值是相对而非可直接兑现的收益；且沪深 300 成分与质押用的是当前快照（无历史数据），这两维仍有轻微残留前视。默认搜索 1500 次，运行时间取决于本地缓存规模。
...
游资雷达从框架升级为可用的 `/radar` 页面，新增吸筹分/出货分、游资形态 playbook、题材热度、市值列、K 线详情、隐藏出货预警、含大盘切换和 TopN txt 导出，并用 `distribution` 模式对出货分的 technical/P11/P19/P20/龙虎榜/divergence 组合做网格搜索与消融实验，用 `accumulation` 模式对 chip/position/cmf_eff/P3/股东户数变化/公司回购六个候选特征做 train/validation 网格搜索与消融实验，线上吸筹和出货总分不再用截面 rank 合成，且线上推荐权重要求每个候选指标至少保留 0.1。A 股策略台长线候选和优化器统一使用 SW3 细分行业龙头池，市值/指数相关因子默认权重为 0 且不参与搜索，折样本时间权重调整为 1.0x-1.0x 等权；长线新增“策略走势”，长线/短线新增导出和固定周 K 小图。补充申万二级板块 SQLite、题材候选、行业周期引擎、成交量单位修复工具，并扩展 Flask 与核心逻辑测试。

#### Update v3.3.1  2026.6.22
修正 A 股长线策略与 optimizer 的实盘一致性：长线默认输出数量改为 20 并抽成常量，optimizer 默认搜索次数改为 1000，历史折权重固定为 1.0x-1.0x 等权，PIT 选股阶段要求 as_of 当天真实可交易，回测路径改用日收益前缀并过滤异常/不可交易路径。长线“策略走势”改为每次按当前参数重新生成，换手统计和图内持仓展示跟随 TopN，避免 Top20 策略仍显示 Top10 的误导。游资雷达继续收敛机会分口径，导出改为机会分 TopN，补充二级行业热度、市值、相似行业和出货预警展示；吸筹/出货相关实验、股东户数/回购/资金爬虫、腾讯/新浪/东财行情 fallback 顺序、强制细分龙头名单以及测试覆盖同步更新。

#### Update v3.3.2  2026.6.25
优化 A 股策略与 optimizer 的性能链路：长线因子计算增加数值字段预转换、numpy 数组缓存和向量化打分，optimizer 预构 PIT 因子矩阵并支持 `OPTUNA_TRIAL_JOBS` 环境变量实验并行，默认搜索次数调整为 1500。刷新链路减少日线 refetch 的整表重写和 stock_meta JSON 往返，价格估值爬虫纯日线 append 走增量 upsert，`sw3_member.market_cap_yi` 改为整轮结束后批量同步；SQLite 连接增加临时表内存化、cache_size 与 WAL checkpoint 调优。同步补充批量市值同步测试，降低大规模刷新与参数搜索的重复计算和写放大。

#### Update v3.2.0.dev3  2026.6.26
调整 optimizer 优化参数落盘策略：`stock_strategy_optimizer.py` 只写入 `data/stock_strategy_optimized_config.json`，不再主动同步写入 `meta_data_backup`；现有备份文件保留，Dashboard 在 data 参数缺失时仍可 fallback 读取备份参数。
