...
- `fund/funds.py`：配置基金列表、持仓列表和比较基准。
- `fund/fund_fetch_data.py`：统一抓取历史净值和实时估算。
- `fund/fund_storage.py`：基金 SQLite 核心缓存，保存历史净值、实时估算和 Scrapy 基金概况快照。
- `fund/fund_technical_analysis.py`：生成技术指标和买卖信号。
- `fund/fund_generate_output.py`：生成 `data/fund_report_data.json`（HTML 由 Flask `/fund` 页面渲染）。
...
`fund_data_refresh.py` 继续作为根目录的跨平台刷新入口，内部基金子步骤从 `fund/` 包以 `python -m fund...` 启动。网页“刷新数据”固定以直连模式启动刷新器；刷新器始终复用当前 Python 解释器，并通过 `python -m scrapy` 调用 Scrapy。直连模式会同时关闭 Requests 的环境代理读取和 Scrapy 的 HTTP 代理中间件，而不只是删除常见代理变量。
...
`stock_radar_fresh_data.py` 是跨平台全量刷新入口，不按候选池拆分：每次都会刷新 `stock_etf_pool.py` 中的 ETF 行情、全量股票、申万二级板块历史和题材候选。ETF 仅抓取行情，不抓取财务、股东户数、回购、龙虎榜等公司层数据。刷新器最后重建默认 `leader` 结果；切到 `hotmoney` 或 `etf` 后需再运行一次雷达，池选择不会缩小底层刷新范围。
...
- “形态回测”的普通买点为 P1/P2/P3/P5/P21/P23/P24/P25 任意一个命中；P23/P25 已按当前产品口径纳入核心有效集，并标记为绿色买点。
...
├── run.py                         # Flask 本地启动入口
├── refresh_workflow.py            # 基金/股票共用的子进程环境、日志和编排
├── fund/                          # 基金配置、存储、抓取、技术分析和报告包
│   ├── funds.py                   # 基金列表和基准配置
│   ├── fund_fetch_data.py         # 基金净值与实时估算抓取
│   ├── fund_storage.py            # 基金 SQLite schema 与读写
│   ├── fund_technical_analysis.py # 基金技术指标和信号
│   ├── fund_generate_output.py    # 基金报告数据生成
│   └── ttjj_spider/               # 天天基金 Scrapy 项目
├── fund_data_refresh.py           # 基金跨平台全量刷新入口
├── scrapy.cfg                     # 根目录 Scrapy 入口，指向 fund.ttjj_spider
...
- 境内行情接口通过本地代理有时会失败；网页刷新默认直连，CLI 可使用 `--no-proxy`，也可设置 `FUND_CRAWL_NO_PROXY=1` 或 `STOCK_CRAWL_NO_PROXY=1`。直连环境会按大小写不敏感方式清理 HTTP、HTTPS、ALL_PROXY，并覆盖当前进程及其子进程。
...
主力资金雷达扩展为龙头、游资小盘和 ETF 三池，新增形态回放、潜伏观察、盘中重算和导出；上线 SW3 三级行业热度与最新日完整性校验，统一基金/股票跨平台刷新和后台任务锁，并补强行情重试、证券隔离、前复权实时投影和滚动五年股息口径。

#### Update v3.4.1  2026.7.19
基金模块迁入 `fund/` 包并统一 Requests、Scrapy 与子进程的直连刷新；股票刷新新增上市日期缓存、历史区间约束和逐股失败报告。游资雷达新增命中形态多选及“全选买入 / 全选卖出（含 P11）/ 全选拉升”，A 股策略台同步 P11/P12/P13/P23/P25 配色并将龙虎榜提示改为“波动加剧”；清理、改写过时测试后，368 项测试全部通过。
