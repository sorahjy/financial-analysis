...
细分龙头行情子进程通过临时 JSON sidecar 把逐股失败交回编排器，父进程再写入 `data/stock_data_refresh_report.json`。每只股票在单轮并发抓取中只处理一次；抓取异常、空行情和单线程写库异常会在写线程结束后按原池顺序汇总。报告顶层 `failures[]` 与对应步骤的 `meta.failure_details[]` 保留本轮失败的 `code`、`name`、`stage`、`error`，CLI 在完整 JSON 后逐条输出全部失败明细。子进程异常和空股票池仍作为无法归属到单股的全局失败。sidecar 使用独立临时目录和参数列表传递，兼容 Windows 路径与文件锁规则。
...
A 股日线统一经过 `stock_crawl_common.fetch_qfq_daily_records()`，按新浪、腾讯、东财的顺序故障转移；各适配器都输出前复权 OHLCV，成交量统一为“手”、成交额统一为“元”，涨跌幅按相邻前复权收盘校正。刷新器先批量读取 `stock_meta.listing_date`，仅对缺失股票调用交易所批量清单并持久化结果，随后把所有历史请求起点收紧到不早于上市日。上市日期获取或代码匹配失败归入 `listing_date` 阶段，不再把上市前空区间误判为行情源失败。连续失败的数据源只会在本轮暂时熔断，批次结束后不会再对失败股票发起第二轮请求。ETF 先走基金专用东财接口，再回退通用源，且不会进入股票上市日期补齐链路。
...
| 表 | 作用 |
| --- | --- |
| `stock_meta` | 以六位代码为稳定主键；保存名称、`instrument_type`、`listing_date`、抓取元数据、基本面/分红等 JSON blob |
...
`data/stock_data_refresh_report.json` 是跨步骤刷新审计记录。除命令、退出码、耗时和健康检查外，它会汇总本轮失败股票的证券代码、名称、失败阶段与异常内容，并由 CLI 在任务末尾输出全部明细；上市日期获取或匹配失败使用阶段名 `listing_date`。无法归属到单股的股票池构建或编排异常使用 `code: null`。策略产物只有在所有依赖步骤成功后才允许重建。
